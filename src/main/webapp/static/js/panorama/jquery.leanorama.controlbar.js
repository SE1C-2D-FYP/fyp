...
$.fn.leanorama.extensions.push(function start() {
    ...
    function create_controlbar() {
        ...
        $controls = {
            show:       div({class: 'leanorama-controlbar-button'}).html('<i class="fa fa-angle-double-right"></i>')   // Show bar
                        .css('display', 'none')
                        .css('margin-left', '.5em'),
            hide:       div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-angle-double-left"></i>'),  // Hide bar
            delim:      div({class: 'leanorama-controlbar-button'}).html('|'),  // Buttons delimiter
            zoomin:     div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-search-plus"></i>'),  // Zoom in
            zoomout:    div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-search-minus"></i>'),  // Zoom out
            play:       div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-play"></i>'),  // Start autorotate
            stop:       div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-stop"></i>'),  // Stop autorotate
            up:         div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-arrow-up"></i>'),  // Look up
            down:       div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-arrow-down"></i>'),  // Look down
            left:       div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-arrow-left"></i>'),  // Look left
            right:      div({class: 'leanorama-controlbar-button actionable'}).html('<i class="fa fa-arrow-right"></i>'),  // Look right
